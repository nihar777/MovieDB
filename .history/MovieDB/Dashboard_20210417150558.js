...
const Dashboard = ({ navigation }) => {
    ...
    const [SearchedMovieData,SetSearchedData]=useState("")
    ...
    function SearchMovie(SearchValue) {
        ...
        axios.get(`https://api.themoviedb.org/3/search/movie?api_key=6f5fae224e38fb668c76e3f839b4d12f&language=en-US&query=${SearchValue}&page=1&include_adult=false`)
            ...
            .then(responce => {
                ...
                SetSearchedData(responce.data.results)
        ...
        <View style={{ flex: 1, flexDirection: "column" }} >
            <View style={{ flex: 0.09 }}>
                ...
                <Header searchBar rounded>
                    <Item>
                        <Input  onChangeText={(SearchData) =>{ SetSearchValue(SearchData)
                            if(SearchData==null){
                                TopRatedMovie()
                               
                            }else{
                                SetMovieData(SearchedMovieData,console.log("After Search=>",MovieDataResponce))

                            }
                            SetInputValue(SearchData)}} 
                            
                            onSubmitEditing={()=>SearchMovie(InputValue)}
                            placeholder="Search" />
                        <TouchableOpacity
